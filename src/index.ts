...
function checkEnvConfig(): EnvConfig {
...
async function getWalletBalanceUsdc(client: ClobClient): Promise<number | null> {
  try {
...
async function createClobClient(config: EnvConfig): Promise<ClobClient> {
...
async function fetchLeaderTrades(leaderAddress: string, limit = 50): Promise<LeaderTrade[]> {
...
function isValidTrade(t: unknown): t is LeaderTrade {
...
function passesMarketFilter(trade: LeaderTrade, filters: string[]): boolean {
  if (filters.length === 0) return true;
  return filters.some((f) => trade.slug.startsWith(f));
}

function tradeKey(trade: LeaderTrade): string {
  return trade.transactionHash;
}

function computeCopyAmount(trade: LeaderTrade, config: EnvConfig): { usd: number; shares: number } {
...
function maxBuyPrice(leaderPrice: number, slippageBps: number): number {
  return Math.min(0.99, leaderPrice * (1 + slippageBps / 10000));
}

function minSellPrice(leaderPrice: number, slippageBps: number): number {
...
function simulateSell(
  ...
  if (pos.shares <= 0.0001) positions.delete(trade.asset);
...
async function copyTrade(
  ...
  if (config.dryRun) {
    if (trade.side === "BUY") simulateBuy(state, positions, trade, usd, shares);
    else simulateSell(state, positions, trade, shares);
    return;
  }
...
function printFinalResult(state: BotState) {
...
async function main() {
  ...
  const startBalance =
    walletBalance != null && walletBalance >= 0 ? Math.floor(walletBalance * 100) / 100 : DEFAULT_START_BALANCE;
  ...
  for (const t of seedTrades) seenTrades.add(tradeKey(t));
